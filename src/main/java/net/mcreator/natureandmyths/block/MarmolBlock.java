...
public class MarmolBlock extends NatureandmythsModElements.ModElement {
	@ObjectHolder("natureandmyths:marmol")
	public static final Block block = null;
	public MarmolBlock(NatureandmythsModElements instance) {
		super(instance, 88);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("marmol", "marmol", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 1)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(20, 40, 40, 64))));
		}
	}
}
